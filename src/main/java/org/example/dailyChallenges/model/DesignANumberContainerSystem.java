...
public class DesignANumberContainerSystem {
    ...
    public void change(int index, int number) {
        ...
        numberToIndices.computeIfAbsent(number, num -> new PriorityQueue<>()).add(index);
    }

    public int find(int number) {
        if (!numberToIndices.containsKey(number)) return -1;

        PriorityQueue<Integer> queue = numberToIndices.get(number);
        while (!queue.isEmpty()) {
            int currentIndex = queue.peek();
            if (indexToNumber.get(currentIndex) == number) {
                return currentIndex;
            }
            queue.poll();
        }
